...
; Copy the visible tile attributes alongside CopyMapViewToVRAM while the LCD is
; disabled. de is the same BG-map destination used for the tile copy.
CopyFullColorMapViewAttributes::
	...
	ld a, 2
	ldh [rSVBK], a
	ld a, [wFullColorOverworldActive]
	and a
	jr z, .restoreWRAMBank
	ld a, 1
	ldh [rVBK], a
	ld hl, wTileMap
	ld b, SCREEN_HEIGHT
...
.column
	...
	ld a, [hli]
	ld c, a
	ld b, HIGH(wFullColorTileAttributes)
	ld a, [bc]
	and $7
	ld [de], a
	pop bc
...
; Mirror a scheduled overworld row/column redraw into VRAM bank 1. Only the
; bounded exposed strip is written during VBlank.
